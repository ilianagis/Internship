document.querySelectorAll('.faq__qna-item').forEach((item) => {
  item.addEventListener('click', () => {
    ...
    if (isActive) {
      ...
      svg.setAttribute('href', '/__spritemap#sprite-plus'); // устанавливает SVG для закрытого состояния
    } else {
      item.classList.add('faq__qna-item--active');
      item.querySelector('.faq__answer').classList.add('faq__answer--active');
      svg.setAttribute('href', '/__spritemap#sprite-minus'); // устанавливает SVG для открытого состояния
    }
  });
});
